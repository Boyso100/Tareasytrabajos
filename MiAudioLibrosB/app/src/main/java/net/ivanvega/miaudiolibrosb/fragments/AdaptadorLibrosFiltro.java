...
/**
 * Created by BOYSO996 on 01/10/2017.
 */
...
public class AdaptadorLibrosFiltro extends AdaptadorLibros {
    ...
    public void setNovedad(boolean novedad) {
        this.novedad = novedad;   recalculaFiltro();
    }

    public void setLeido(boolean leido) {
        this.leido = leido;   recalculaFiltro();
    }
    ...
    public void recalculaFiltro() {
        ...
        for (int i = 0; i < vectorSinFiltro.size(); i++) {
            ...
            if ((libro.titulo.toLowerCase().contains(busqueda) || libro.autor.toLowerCase().contains(busqueda))
                    && (libro.genero.startsWith(genero)) && (!novedad || (novedad && libro.novedad))
